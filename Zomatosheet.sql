...
select city, left(avg(rating),4) as  avgrating from sheet1
group by City
order by avgrating desc;

select city,  left(avg(rating),4) as avgrating
from (select city, Rating , country_name from sheet2 where Countryname='india' )
as indiaonly
group by city
order by avgrating desc;

select ifnull(Cuisines,'-') as cuisines, left(avg(Rating),4) as  avgrating
from cuisines as c join sheet1 as z on c.RestaurantID=z.RestaurantID
group by cuisines
order by avgrating desc;
